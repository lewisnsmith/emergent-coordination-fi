...
[Metrics](metrics.md); lower dispersion = more convergence).
...
- **H13 (local-model fidelity and quantization propagation).** Under paired prompts and financial
  tasks, model scale, checkpoint family, precision, reasoning depth, and state feedback may change
  behavioral equivalence to frontier models and the rate at which verifiable reasoning errors
  become decision, portfolio, and market-path differences. H13 is a future protocol, not a cheap
  substitute for H1 or evidence that a frontier model is ground truth.

The machine-readable catalog is
[`configs/research-program.yaml`](../../configs/research-program.yaml), which maps the broader
H1–H13 program (including H2b) to `exp-000` through `exp-026`. The narrower first-paper claim and
analysis contract is frozen through [Preregistration](preregistration.md).
...
- H13 uses three noninterchangeable references: an executable financial oracle for correctness,
