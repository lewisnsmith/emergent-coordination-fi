            ...
            rows.append(
                {
                    **_identity(block),
                    "metric": "kappa",
                    "component": component,
                    "pair_id": f"pair-{component}",
                    "code_same": "".join(same),
                    "code_different": "00000",
                    "value_same": 0.8,
...
def test_crossed_design_recovers_hand_computed_factorial_effects():
    ...
    assert _estimate(result, "H1.technology.homogeneous") == pytest.approx(1.5)
    assert _estimate(result, "H1.technology.heterogeneous") == pytest.approx(2.5)
    assert _estimate(result, "H1.technology_x_ecology") == pytest.approx(1.0)
    assert set(result.effects["independent_n"]) == {2}
    assert len(result.block_effects) == 6
...
def test_factorial_contrasts_do_not_mistake_ecology_for_technology():
    ...
    assert _estimate(result, "H1.technology.homogeneous") == 0.0
    assert _estimate(result, "H1.technology.heterogeneous") == 0.0
    assert _estimate(result, "H1.technology_x_ecology") == 0.0
...
def test_h3_and_h4_are_aggregated_to_one_effect_per_block_and_share_holm_family():
    crossed = _crossed_rows(
        {
            "b1": (2.0, 5.0, 1.0, 3.0),
            "b2": (4.0, 7.0, 2.0, 4.0),
    ...
    assert len(result.multiplicity["frozen_contrasts"]) == 12
    assert all(result.effects["p_adjusted"] >= result.effects["p_value"])
    assert set(result.effects["independent_n"]) == {2}
